describe("Create, edit and delete emoji", () => {
  ...
  it("Create, edit and delete emoji", () => {
    ...
    cy.get("#emoji-table-body").then(($body) => {
      const rowsBefore = $body.children();
      const initialCount = rowsBefore.length || 0;

      // Upload emoji
      cy.get("#upload-emoji-button").click();
      cy.get("#emoijImage").selectFile(`cypress/fixtures/emoji.jpg`, {
        force: true
      });

      cy.wait("@uploadEmojiRequest")
      ...
      cy.wait("@editEmojiRequest").its("response.statusCode").should("eq", 200);

      // Delete emoji
      cy.intercept("DELETE", `**/api/guilds/*/emojis/*`).as(
        "deleteEmojiRequest"
      );
