...
class GoogleFeeder(Feeder):
            ...
            if date == "anytime":
                ...
                for date_ in date:
                    if date_ is None:
                        ...
                        date_ = (
                            datetime.date(*date_) if isinstance(date_, tuple) else date_
                        )
    ...
    def feed(self, keyword, offset, max_num, language=None, filters=None):
        ...
        for i in range(offset, offset + max_num, 100):
            params = dict(
                q=keyword, ijn=int(i / 100), start=i, tbs=filter_str, tbm="isch"
            )
            if language:
...
class GoogleParser(Parser):
    def parse(self, response):
        ...
        for div in image_divs:
            ...
            uris = re.findall(r"http[^\[]*?.(?:jpg|png|bmp|gif)", txt)
            if not uris:
                uris = re.findall(r"http[^\[]*?\.(?:jpg|png|bmp|gif)", txt)
...
class GoogleImageCrawler(Crawler):
    ...
    def crawl(
        ...
        if offset + max_num > 1000:
            if offset > 1000:
                self.logger.error(
                    "Offset cannot exceed 1000, otherwise you "
