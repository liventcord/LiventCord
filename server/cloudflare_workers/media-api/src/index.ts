...
export default {
  async fetch(request: Request, env: any, ctx: any): Promise<Response> {
    ...
    if (previewMatch) return handlePreview(request, env);
    ...
    async function fetchFile(
      ...
      try {
        ...
        const conditions = Object.keys(filters)
          .map((col, i) => {
            const prefix = baseColumns.has(col.toLowerCase()) ? "b" : "f";
            return `"${prefix}"."${col}" = $${i + 1}`;
          })
          .join(" AND ");
    ...
    `;
        ...
        if (version && versionedTables.has(tableName)) {
          query += ` AND f."Version" = $${params.length + 1}`;
          params.push(version);
        } else if (versionedTables.has(tableName)) {
        ...
        if (!content)
          return { ok: false, status: 410, error: "File content unavailable" };
    ...
    async function getFromCache(key: string, fetcher?: () => Promise<any>) {
      ...
      const response = new Response(file.content, { status: 200, headers });
      try {
    ...
    if (pathParts[0] === "guilds") {
      ...
      if (pathParts[2] === "emojis" && pathParts[3]) {
        const fileId = sanitizeId(stripExtension(pathParts[3]));
        return getFromCache(`emoji_${guildId}_${fileId}`, () =>
          fetchFile("EmojiFile", { GuildId: guildId, FileId: fileId }),
        );
      }
